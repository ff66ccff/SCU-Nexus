...
JwxtScheduleParser::ParseResult JwxtScheduleParser::parse(const QJsonObject& rawJson) {
    ...
    int colorIndex = 0;
    auto failImport = [&](const QString& detail) {
        result.courses.clear();
        result.warnings.append(detail);
        result.errorMessage = QStringLiteral("导入数据格式异常");
        qWarning().noquote() << "Schedule import parse failure:" << detail;
        return result;
    };

    // Iterate xkxx array
    for (const auto& xkxxItem : xkxx) {
        if (!xkxxItem.isObject()) {
            return failImport(QStringLiteral("xkxx 中存在非对象条目"));
        }
        ...
        for (auto it = xkxxObj.begin(); it != xkxxObj.end(); ++it) {
            if (!it.value().isObject()) {
                return failImport(QStringLiteral("课程条目不是对象：%1").arg(it.key()));
            }
            QJsonObject courseDetail = it.value().toObject();
            if (courseDetail.isEmpty()) {
                return failImport(QStringLiteral("课程条目为空：%1").arg(it.key()));
            }

            // Extract course name and sequence number
            QString courseName = courseDetail["courseName"].toString().trimmed();
            if (courseName.isEmpty()) {
                return failImport(QStringLiteral("课程名称为空：%1").arg(it.key()));
            }
            ...
            if (timePlaceList.isEmpty()) {
                return failImport(
                    QStringLiteral("课程 \"%1\" 没有时间地点信息").arg(displayName));
            }

            for (const auto& tpVal : timePlaceList) {
                if (!tpVal.isObject()) {
                    return failImport(
                        QStringLiteral("课程 \"%1\" 的时间地点条目不是对象").arg(displayName));
                }
                QJsonObject tpObj = tpVal.toObject();
                const QString classWeek = tpObj["classWeek"].toString();
                bool validWeekMask = !classWeek.isEmpty();
                bool hasActiveWeek = false;
                for (const QChar bit : classWeek) {
                    if (bit != QLatin1Char('0') && bit != QLatin1Char('1')) {
                        validWeekMask = false;
                        break;
                    }
                    hasActiveWeek = hasActiveWeek || bit == QLatin1Char('1');
                }
                if (!validWeekMask) {
                    return failImport(
                        QStringLiteral("课程 \"%1\" 的周次掩码无效").arg(displayName));
                }
                if (!hasActiveWeek) {
                    result.warnings.append(
                        QStringLiteral("课程 \"%1\" 的该时间地点没有有效周次，已跳过")
                            .arg(displayName));
                    continue;
                }
                ...
                if (!courseOpt.has_value()) {
                    return failImport(
                        QStringLiteral("课程 \"%1\" 的时间地点字段无效").arg(displayName));
                }
    ...
    const QString& teacher)
{
    ...
    // Basic validation
    if (classDay < 1 || classDay > 7 || classSessions < 1 || continuingSession < 1) {
        return std::nullopt;
    }
